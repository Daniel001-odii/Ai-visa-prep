<template>
  <div class="flex flex-col p-5 min-h-screen justify-center items-center">
    ...
    <div class=" flex justify-center items-center flex-col">
      ...
      <UButton @click="newInterview()" :loading="loading" :disabled="loading" loading-icon="svg-spinners:bars-rotate-fade" label="Start New Practice" class=" w-fit mt-6" color="blue"/>
    </div>

    <div class="flex flex-wrap gap-3 mt-12">
      <UCard v-for="(stat, index) in user_Stats" :key="index" class=" w-full md:w-fit min-w-[250px]">
        <div class="flex gap-4 items-center">
          <span :class="`bg-${stat.color}-500 text-${stat.color}-500`" class=" bg-opacity-10 p-3 rounded-md flex justify-center items-center">
